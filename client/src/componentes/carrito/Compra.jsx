...
const Compra = ({user}) => {
    ...
    const seleccionarFormaPago = (e) => {
      const formaPagoSeleccionada = e.target.value;
    ...
    initMercadoPago('APP_USR-900b34bc-2297-470d-86db-3ed19e2e2abe');

    const createPreference = async () => {
      try {
      const response = await axios.post("http://localhost:8080/create_preference", {
        description: "Compra Appilcha",
        price: obtenerTotal(),
        quantity: 1
      });
    ...
    const handleBuy = async () => {
        ...
        setPreferenceId(id);
          ...
          Swal.fire({
            position: 'center-center',
            icon: 'warning',
            title: 'Seleccione una forma de retiro',
            showConfirmButton: false,
            timer: 1500
          });
        }
    ...
    return (
        <div className='mt-3'>
            ...
            {formaPago !== 'mp' && (
              <button id='btnPagar' className='btn btn-primary mt-3' onClick={pagar}>
                Pagar
              </button>
            )}
            {formaPago === 'mp' && (
